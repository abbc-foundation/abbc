    ...
    rpcConsole(0)
{
    setMinimumSize(870,620);
            resize(870,620);
    setWindowTitle(tr("ABBCCoin") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/bitcoin"));
    setWindowIcon(QIcon(":icons/bitcoin"));
    qApp->setStyleSheet("QMainWindow { background-image:url(:images/bg);border:none;font-family:'Open Sans,sans-serif'; } #frame { } QToolBar QLabel { padding-left:2px;padding-right:2px;padding-bottom:0px;margin:0px; } #spacer { background-color: qlineargradient(x1: 0, y1: 0, x2: 1, y2: 0, stop: 0 rgb(255,180,26), stop: 1 rgb(110,80,42));border:none; } #toolbar3 { border:none;width:1px; background-color: rgb(0,0,0); } #toolbar2 {border: 0px solid black; background-color:rgb(0,0,0);} #toolbar { border:none;height:100%;padding-top:0px; background-color:qlineargradient(x1: 0, y1: 0, x2: 1, y2: 0,stop: 0 rgb(255,180,26), stop: 1 rgb(110,80,42)); text-align: center; color: white;min-width:150px;max-width:150px;}   QToolBar QToolButton { font-family:Century Gothic;padding-left:20px;padding-right:20px;padding-top:10px;padding-bottom:10px; width:100%; color: white; text-align: center; background-color: qlineargradient(x1: 0, y1: 0, x2: 1, y2: 0,stop: 0 rgb(255,180,26), stop: 1 rgb(110,80,42)); }  QToolBar QToolButton:checked {background-color:rgb(255,180,26);} QToolBar QToolButton:pressed {background-color:rgb(255,180,26);} #labelStakingIcon { padding-left:40px;font-family:Century Gothic;width:100%;font-size:10px;text-align:center;color:white; } QMenu { background: rgb(0,0,0); color:rgb(255,180,26); padding-bottom:10px; } QMenu::item { color:rgb(255,180,26); background-color: transparent; } QMenu::item:selected { background-color:rgb(12,79,143); } QMenuBar { background: rgb(0,0,0); color:rgb(255,180,26); } QMenuBar::item { font-size:12px;padding-bottom:8px;padding-top:8px;padding-left:15px;padding-right:15px;color:rgb(255,180,26); background-color: transparent; } QMenuBar::item:selected { background-color:rgb(12,79,143); } QTableView {background-color: transparent;} SendCoinsEntry {background-color: transparent; border: 1px solid rgb(179,179,179);}");
#else
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a ABBCCoin address"));
    ...
    aboABBCCointion = new QAction(QIcon(":/icons/bitcoin"), tr("&About ABBCCoin"), this);
    aboABBCCointion->setToolTip(tr("Show information about ABBCCoin"));
    aboABBCCointion->setMenuRole(QAction::AboutRole);
    ...
    optionsAction->setToolTip(tr("Modify configuration options for ABBCCoin"));
    ...
    connect(aboABBCCointion, SIGNAL(triggered()), this, SLOT(aboutClicked()));
...
void BitcoinGUI::createMenuBar()
{
    ...
    help->addSeparator();
    help->addAction(aboABBCCointion);
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("ABBCCoin client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/toolbar_testnet"));
                toggleHideAction->setIcon(QIcon(":/icons/toolbar_testnet"));
            }

            aboABBCCointion->setIcon(QIcon(":/icons/toolbar_testnet"));
        }
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("ABBCCoin client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to ABBCCoin network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid ABBCCoin address or malformed URI parameters."));
    }
