...
const string strMessageMagic = "ABBCCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // ABBCCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        if (txout.nValue > MAX_MONEY)
            return DoS(100, error("CTransaction::CheckTransaction() : txout.nValue too high %s",txout.nValue ));
                        ...
                        bool* pfMissingInputs)
{
    ...
    // ABBCCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if (pindexBest->nHeight+1 == 1)
    {
	 ...
	 else if (pindexBest->nHeight+1 < 259201)
    {
      nSubsidy = 20 * COIN;
      return nSubsidy + nFees;
    }
    else if (pindexBest->nHeight+1 < 518401)
    {
      nSubsidy = 10 * COIN;
      return nSubsidy + nFees;
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, unsigned int nTime, int64_t nFees, int64_t nValueIn, uint256 prevHash)
{
    ...
    if(int(pindexBest->nHeight) >= 466310){
	...
	if (inputcoins >= 25000000) 
	{
		...
		if(rand2 <= 4)
			nBonusSubsidy += 200 * COIN;
		if(rand3 <= 3)
			nBonusSubsidy += 300 * COIN;
		if(rand4 <= 2)
			nBonusSubsidy += 500 * COIN;
		if(rand5 <= 1)
			nBonusSubsidy += 1000 * COIN;
...
// ABBCCoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // ABBCCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // ABBCCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // ABBCCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // ABBCCoin: track money supply and mint amount info
...
// ABBCCoin: total coin age spent in transaction, in the unit of coin-days.
...
// ABBCCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // ABBCCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // ABBCCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // ABBCCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // ABBCCoin: compute stake modifier
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
        ...
        // error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016"PRIx64, pindexNew->nHeight, nStakeModifier);
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (GetBlockTime() > GetAdjustedTime() + GetClockDrift(GetBlockTime()))
        return error("CheckBlock() : block timestamp too far in the future GetBlockTime=%" PRId64 " GetAdjustedTime=%" PRId64 " GetClockDrift=%" PRId64,GetBlockTime(),GetAdjustedTime(),GetClockDrift(GetBlockTime()));
    ...
    if (IsProofOfStake())
    {
        ...
        // ABBCCoin: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // ABBCCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // ABBCCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // ABBCCoin: getblocks may not obtain the ancestor block rejected
    ...
    // ABBCCoin: if responsible for sync-checkpoint send it
...
// ABBCCoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ABBCCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // ABBCCoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // ABBCCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // ABBCCoin: tell downloading node about the latest block if it's
